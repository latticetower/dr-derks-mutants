...
class DerkAgent(object):
    ...
    def __init__(self, n_agents, action_space, estimator, epsilon=0.0):
        """
        Parameters:
         - n_agents: TOTAL number of agents being controlled (= #arenas * #agents per arena)
        """
    ...
    def take_action(self, env_step_ret):  # TODO estimate all actions using epsilon
        """
        Parameters:
         - env_step_ret: whatever env.step() returned (obs_n, rew_n, done_n, info_n)
        ...
        # перебрать все варианты ходов, выбрать наилучший
        # если значение epsilon > 0.0 то с некоторой вероятностью выбирать рандомно

        action_n = [self.estimator(observation_n[i]) for i in range(self.n_agents)]
